...
%     popSize = size(avg_cell_table.perturbVecs,2);
%     if popSize < 100
...
%       passed = avg_cell_table.perturbVecs(:,1:N); % perturbVecs is sorted
%       p_ind = randsample(N,3); % Three parents
%       p(:,1) = avg_cell_table.perturbVecs(:,min(p_ind)); % best parent
%       p(:,2) = avg_cell_table.perturbVecs(:,median(p_ind)); % mid parent
%       p(:,3) = avg_cell_table.perturbVecs(:,max(p_ind)); % worst parent
